...
expect(canRedeem(11)).toBe(true);  // 邊界內</pre>`;
  ...
  function applyBeginnerGuides() {
    const guides = {
      1: {
        problem: '程式現在可能還能跑，但未來每次改需求都很痛，因為責任混在一起、重複邏輯很多、命名看不懂。',
        principle: 'Code Smell 不是 bug，而是維護風險訊號。它提醒你：這段程式的結構可能會讓下一次修改更慢、更容易改壞。',
        read: '先不要背所有 smell 名稱。先抓一件事：如果一個小需求會逼你改很多地方，或讀一段程式要理解很多不相關事情，它就可能有味道。',
        analogyTitle: '房間還能住，但收納很亂',
        analogy: [
          ['房間還能住', '程式功能目前還能運作'],
          ['東西都堆在同一個箱子', 'God Object 或函式責任過多'],
          ['找鑰匙要翻整個房間', '改小需求也要讀很多無關程式'],
          ['用抽屜分類', '重構成清楚的類別、函式、值物件']
        ]
      },
      2: {
        problem: '功能看起來成功，不代表真的正確。畫面顯示成功，但資料庫、金額、狀態、通知可能已經錯了。',
        principle: '功能正確性是在指定條件下，系統產生符合規格的結果。測試要檢查「結果」和「狀態變化」，不是只看有沒有回應。',
        read: '第一次讀只要抓住：測試不是「跑過就好」，而是要用斷言檢查 happy path、sad path、evil path。',
        analogyTitle: 'ATM 提款不能只看螢幕',
        analogy: [
          ['螢幕顯示提款成功', 'API 回傳成功'],
          ['帳戶餘額正確減少', '資料庫狀態正確'],
          ['吐鈔機真的吐出錢', '外部副作用正確'],
          ['交易紀錄存在', 'audit log 或事件紀錄正確']
        ]
      },
      3: {
        problem: '很多錯誤不是發生在普通輸入，而是發生在剛好踩線的地方，例如滿 10、等於 18、最大長度 255。',
        principle: '邊界測試就是刻意測規則切換點。因為工程師最容易把 <code>&gt;=</code>、<code>&gt;</code>、<code>&lt;=</code>、<code>&lt;</code> 寫錯。',
        read: '看到任何規則，先問三個值：邊界前一格、邊界本身、邊界後一格。',
        analogyTitle: '遊樂園身高閘口',
        analogy: [
          ['119 公分不能進', '邊界外側'],
          ['120 公分剛好能進', '邊界點，最重要'],
          ['121 公分能進', '邊界內側'],
          ['感測器差 1 公分就出事', '比較運算子或單位轉換寫錯']
        ]
      },
      4: {
        problem: '如果所有測試都只靠瀏覽器完整流程，測試會很慢、很脆弱，失敗時也不知道哪裡壞。',
        principle: '測試金字塔是在平衡速度、成本和信心。越底層越快越便宜，越上層越接近真實使用者但越慢。',
        read: '不要把金字塔當死比例。重點是：商業規則盡量用快測試保護，最關鍵使用者旅程才放 E2E。',
        analogyTitle: '車子不是只靠試駕品管',
        analogy: [
          ['測每顆螺絲', 'Unit Test，快且定位精準'],
          ['測引擎和變速箱合裝', 'Integration Test，檢查協作'],
          ['整車上路試駕', 'E2E Test，最接近使用者但成本高'],
          ['每個問題都等試駕才發現', '測試冰淇淋筒反模式']
        ]
      },
      5: {
        problem: 'API 手動按一次看起來正常，但下次後端改欄位時，沒有人會自動提醒前端壞掉。',
        principle: 'Postman 把 API 請求和斷言整理成 Collection；Newman 讓這份 Collection 在 CI 自動執行。',
        read: '先分清楚三層：request 是一次呼叫，tests 是檢查規則，collection 是整份檢查表。',
        analogyTitle: '餐廳菜品檢查表',
        analogy: [
          ['主廚試做一道菜', 'Postman 手動發 request'],
          ['菜品檢查表', 'Collection'],
          ['重量、溫度、成分規則', 'Tests / Assertions'],
          ['每批出餐自動檢查', 'Newman 在 CI 執行']
        ]
      },
      6: {
        problem: '測試覆蓋率很高，但測試可能只是在跑程式，沒有真的驗證結果。',
        principle: '突變測試故意把程式改壞，再看測試會不會失敗。會失敗表示測試抓得到錯，不失敗表示測試保護力不足。',
        read: '先記住 killed 和 survived：killed 是好事，代表測試抓到錯；survived 是警訊，代表錯誤版本逃過測試。',
        analogyTitle: '防彈衣不是摸過就算安全',
        analogy: [
          ['確認防彈衣有穿上', '覆蓋率：程式有被執行'],
          ['朝薄弱處開槍', '注入突變體'],
          ['子彈被擋下', 'Killed，測試抓到錯'],
          ['子彈穿過沒警報', 'Survived，測試太弱']
        ]
      },
      7: {
        problem: '很多人以為壓測就是開很多使用者，但真正重要的是流量怎麼進來、延遲是否可接受、錯誤率是否超標。',
        principle: 'k6 用腳本描述負載情境，並用 thresholds 讓壓測自動判斷通過或失敗。',
        read: '先不要問「幾個 VU」。先問：我要找容量上限、驗證 SLA，還是測突發流量？',
        analogyTitle: '捷運站尖峰人流',
        analogy: [
          ['站內正在移動的人', 'VU'],
          ['每分鐘新進站的人', 'arrival rate'],
          ['從進站到上車花多久', 'latency'],
          ['等待超過 10 分鐘就不合格', 'threshold']
        ]
      },
      8: {
        problem: '功能沒有壞，但變慢了。使用者仍能完成操作，只是每次都多等一點，久了就變成產品問題。',
        principle: '效能退化測試把新版本和基準線比較，檢查延遲、CPU、記憶體或吞吐量是否明顯變差。',
        read: '重點不是一次測到幾毫秒，而是跟以前穩定版本比，有沒有顯著變慢。',
        analogyTitle: '跑步成績突然退步',
        analogy: [
          ['過去平均配速', 'Baseline'],
          ['今天突然慢很多', 'Performance Regression'],
          ['天氣、路線、睡眠', '測試環境變因'],
          ['連續多次確認', '避免把自然波動誤判成退化']
        ]
      },
      9: {
        problem: '系統慢了，但靠猜通常猜錯。你以為是資料庫，其實可能是序列化、正規表示式、鎖、記憶體配置。',
        principle: 'Profiling 是量測程式把時間或資源花在哪裡。先量測，再優化。',
        read: '看到 flame graph 時，先看寬度。越寬表示消耗比例越高，越值得先查。',
        analogyTitle: '醫生先照影像再開刀',
        analogy: [
          ['病人說胸悶', '使用者說系統慢'],
          ['心臟超音波或檢查', 'Profiler'],
          ['找到堵塞血管', '定位瓶頸函式'],
      ...
      10: {
        problem: '系統上線後，你不能靠使用者回報才知道壞了。你需要持續看到請求數、錯誤率、延遲、資源使用。',
        principle: 'Prometheus 會定期抓取 metrics，保存成時間序列，讓你能查趨勢與設定告警。',
        read: '先分清楚 metric 和 label。metric 是數字，label 是切分維度。label 太細會造成高基數災難。',
        analogyTitle: '全國氣象站',
        analogy: [
          ['各地氣象站', '每個服務或 pod'],
          ['定期回報溫度雨量', 'scrape metrics'],
          ['縣市、測站類型', '低基數 label'],
          ['每個觀測員身分證', '高基數 label，應避免']
        ]
      },
      11: {
        problem: '有資料不代表看得懂。Prometheus 有數字，但值班工程師需要一眼知道服務健不健康、哪裡壞、影響多大。',
        principle: 'Grafana 把資料來源變成 dashboard panel，協助觀察趨勢、比較維度、排查問題。',
        read: '不要把 Grafana 想成資料庫。它主要是視覺化和查詢入口，資料通常來自 Prometheus、Loki、Tempo 或 SQL。',
        analogyTitle: '飛機玻璃駕艙',
        analogy: [
          ['飛機感測器', 'Prometheus / logs / traces'],
          ['駕駛艙螢幕', 'Grafana dashboard'],
          ['高度速度油量', 'panel 指標'],
          ['警示燈', 'alert 或異常視覺提示']
        ]
      },
      12: {
        problem: '只追求高吞吐量、低延遲或高資源利用率其中一個，常常會害系統失衡。',
        principle: '吞吐量、延遲、資源利用率互相牽制。利用率接近飽和時，排隊會讓延遲急速上升。',
        read: '先抓一句話：系統越接近滿載，不是只是慢一點，而是可能開始排隊爆炸。',
        analogyTitle: '咖啡店尖峰排隊',
        analogy: [
          ['每分鐘做幾杯咖啡', 'Throughput'],
          ['客人等多久拿到咖啡', 'Latency'],
          ['咖啡師忙碌比例', 'Utilization'],
    ...
    concepts.forEach((node) => {
      const guide = guides[node.id];
      if (!guide || node._beginnerGuideApplied) return;
      node._beginnerGuideApplied = true;
      node.intro = renderBeginnerGuide(guide) + (node.intro || '');
      node.analogies = [
        {
          icon: '🧭',
          title: `小白總類比：${guide.analogyTitle}`,
          body: renderAnalogyMapping(guide.analogy)
        },
        ...(node.analogies || [])
      ];
    });
  ...
  function renderBeginnerGuide(guide) {
    return `
      <div style="border:1px solid rgba(56,189,248,0.28);background:rgba(8,47,73,0.20);padding:12px;border-radius:8px;margin-bottom:12px">
        <div style="font-size:0.78em;font-weight:900;color:#7dd3fc;margin-bottom:8px;letter-spacing:.04em">小白版先讀：先懂原理，再看名詞</div>
        <p><b>這頁在解決什麼問題？</b><br>${guide.problem}</p>
        <p><b>背後原理是什麼？</b><br>${guide.principle}</p>
        <p><b>第一次讀要抓哪件事？</b><br>${guide.read}</p>
        <div style="margin-top:8px;padding:9px;border-radius:7px;background:rgba(15,23,42,0.62);border:1px solid #1e293b">
          <b style="color:#fbbf24">生活類比對照：${guide.analogyTitle}</b>
          ${renderAnalogyMapping(guide.analogy)}
        </div>
      </div>`;
  }

  function renderAnalogyMapping(rows) {
    return `
      <div style="display:grid;gap:6px;margin-top:8px">
        ${rows.map(([life, tech]) => `
          <div style="display:grid;grid-template-columns:minmax(86px,.8fr) minmax(0,1.2fr);gap:8px;align-items:start">
            <span style="color:#fbbf24;font-weight:800">${life}</span>
            <span style="color:#cbd5e1">${tech}</span>
          </div>
        `).join('')}
      </div>`;
  }
